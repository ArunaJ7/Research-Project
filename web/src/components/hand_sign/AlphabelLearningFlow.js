          ...
          <div className="row justify-content-center">
            {selectedLesson.images.length > 0 && (
              <div className="col-md-4">
                <img
                  src={selectedLesson.images[0]}
                  alt={`Letter ${selectedLesson.title}`}
            ...
            <div className="col-md-6">
              {snapshots.length > 0 ? (
                <div>
                  ...
                  <div className="result-display my-4">
                    <p className={result === selectedLesson.title ? "text-success" : "text-danger"}>
                      {result === selectedLesson.title 
                        ? `✅ Great job! You correctly signed "${selectedLesson.title}"`
                        : `❌ Detected: "${result}". Keep practicing!`}
                    </p>
                  </div>
                </div>
              ) : (
                <p className="text-muted">No snapshots were taken during practice.</p>
              )}
            </div>
            ...
            <div className="col-md-6">
              <div className="card">
                <div className="card-header bg-primary text-white">
                  <h5>Words with {selectedLesson.title}</h5>
                </div>
                    ...
                    <ul className="list-group list-group-flush">
                      {selectedLesson.examples.map((example, index) => (
                        <li key={index} className="list-group-item d-flex justify-content-between align-items-center">
                          <span style={{ fontSize: '1.5rem' }}>{example.split(' ')[0]}</span>
                          <span className="text-muted">{example.split('(')[1].replace(')', '')}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-muted">No example words available for this letter.</p>
                  )}
                </div>
              </div>
              <div className="mt-3">
