    ...
    jQuery(document).ready(function() {
        $('a[ref="ItemNo"]').click(function() {
            ...
            $.ajax({
                ...
                url: '{{url("/reports/conItem/MonthlyConsumableItems/")}}/' + next + '/{{$year == null ? "-1" : $year}}/{{$month == null ? "-1" : $month}}/{{$valueOfDateCategory}}/{{$fromDate == null ? "None" : $fromDate}}/{{$toDate == null ? "None" : $toDate}}',
        ...
        $('a[ref="hrdAll"]').click(function() {
            ...
            $.ajax({
                type: 'GET',
                url: '{{url("reports/consumables/IssuesMonthly/total")}}/' + next + '/{{$year == null ? "-1" : $year}}/{{$month == null ? "-1" : $month}}/{{$valueOfDateCategory}}/{{$fromDate == null ? "None" : $fromDate}}/{{$toDate == null ? "None" : $toDate}}',
                data: {},
                dataType: 'json',
                success: function(data) {
                    $('#ajaxResults').html(data.html);
